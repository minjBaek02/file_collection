...
- GROUP1 대부분은 한 클러스터로 모이지만, **CP076082.1은 가장 바깥쪽 가지에 위치**
---
...
## 9. Future goal: Follow-up Analysis Plan for **CP076082.1** (Accessory-Gene–Based Tree)

### 1. **Interpretation of CP076082.1 Position**
- GROUP1 대부분은 한 클러스터로 모이지만, **CP076082.1은 가장 바깥쪽 가지에 위치** →  
  **C/G/E accessory 조합은 GROUP1 평균과 유사하지만, 그 안에서는 가장 특이한 패턴**을 가진 균주.
- GROUP1이 전반적으로 **C/G/E-enriched 그룹**이라면, CP076082.1은  
  - CGE 프로파일은 유지하되  
  - **일부 발효·대사 관련 accessory gene이 부족하거나**,  
  - **다른 타입의 accessory가 추가되어 거리 기반 클러스터에서 살짝 벗어난 사례**로 해석 가능.

---

### 2. **Next Steps for Comparative Analysis (이번 주 → 다음 주 목표)**

#### **(1) CGE 카운트 비교**
- 이미 만든 `cog_by_strain_CGE_h35_no_CP033371.tsv`에서  
  **CP076082.1 row만 추출** → GROUP1 나머지 스트레인들과 나란히 비교.
- 주요 확인 포인트:
  - GROUP1 boxplot에서 CP076082.1의 위치  
  - **C/G/E 각각의 카운트가 낮은지 높아 튀는지**  
  - 특히 **C 또는 E만 유난히 낮은 경우**, metabolic pathway 차이 가능성.

#### **(2) Accessory Presence/Absence Matrix 검토**
- GROUP1 내에서 **CP076082.1만 단독으로 가지는 accessory gene**이 있는지 확인.
- 반대로:
  - GROUP1 대부분이 보유하는데  
  - **CP076082.1만 결손된 accessory gene** 목록도 체크.
- 이를 통해:
  - **왜 branch length가 길어졌는지**  
  - 기능적으로 어떤 pathway 또는 stress response가 달라졌는지  
  설명 가능한 요소 찾기.

...
### 🔹 IDEA: C/G/E + M 카테고리를 활용한 “발효 적합 균주(Fermentation-Fit Strain)” 선별

### 1. Observations
- Group 1은 **C/G/E**(대사·에너지·아미노산 관련) 유전자가 가장 풍부하게 나타남.  
- **M 카테고리(COG-M)**는 세포벽·세포막 구조(cell wall/envelope) 관련 기능으로, 발효 환경에서의 **생존·부착·스트레스 내성**에 중요한 역할을 함.

### 2. Rationale / Idea
- 발효 적응에는 **대사 관련 기능(C/G/E)**뿐 아니라,  
  **막/세포표면 구조(M)**도 동일하게 중요 요소로 작용함.  
- 따라서 **C/G/E가 풍부하면서 M 카테고리 유전자도 많은 균주**는  
  발효 환경(산/염 스트레스, 부착, 성장)에 더 적합할 가능성이 높음.  
- → 두 기능군을 결합해 **“발효 유망 균주(Fermentation-Fit Candidate)”**를 탐색해볼 수 있을 듯함.
